...
$(function () {
    ...
    //تفعيل الاختاير بين عناصر ال radio 
    $('.product-option input[type="radio"]').on("change", function () {
        //حذف التفعيل عن الزر المفعل 
        $(this).parents('.product-option').siblings().removeClass('active');
        //اضافه تفعيل للزر المختار 
    ...
    $('#form-checkout select[name="country"]').on( "change",function() {
      ...
      cities.forEach(function(city) {
        ...
        //يحدث القيمه الجديد او يأتي بالقيمه الجديده من القائمة المقابله 
        newOption.text(city);
        //يأتي بالقيمة 
        newOption.val(city);
        //ادخال المحتوى الذي تم جلبه الى قائمة المدن
  ...
  $('#form-checkout input[name="payment_method"]').on( "change",function() {
   ...
   $( "#price-range" ).slider({
    range: true, // هنا بمعنى انه يظهر مقبضين وفي حال كان خطأ يظهر مقبض واحد فقط 
    min: 50,
    max: 1000,
    step: 50,
    values: [ 250, 800 ],
